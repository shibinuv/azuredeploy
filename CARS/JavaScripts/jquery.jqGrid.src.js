...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
        ...
        setPager = function (pgid, tp){
			...
			if(dir=="rtl") { pgl += str; }
			if(ts.p.pginput===true) { pginp= "<td dir='"+dir+"'>"+$.jgrid.format(ts.p.pgtext || "","<input class='ui-pg-input' type='text' size='2' maxlength='7' value='0' role='textbox' disabled='disabled' style='text-align:center'/>","<span id='sp_1'></span>")+"</td>";}
			if(ts.p.pgbuttons===true) {
	...
	editCell : function (iRow,iCol, ed){
	    return this.each(function () {
			...
			if ($t.p.savedRow.length>0) {
				...
				//$($t).jqGrid("saveCell", $t.p.savedRow[0].id, iCol);
			} else {
...
expandSubGridRow: function (rowid) {
    return this.each(function () {
		...
		if(!$t.grid && !rowid) {return;}
		if($t.p.subGrid===true) {
		    var rc = true;//$(this).jqGrid("getInd",rowid,true);
			if(rc) {
