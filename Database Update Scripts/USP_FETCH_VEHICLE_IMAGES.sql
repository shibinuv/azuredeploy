...
--*#0001#    
--'*********************************************************************************'*********************************    
CREATE Procedure [dbo].[USP_FETCH_VEHICLE_IMAGES]
@REGNO		VARCHAR(100)
as  
--update TBL_MAS_VEHICLE_REFNO set refno_count+=1 where refno_code = 2
BEGIN  
	
	select FILE_NAME, FILE_PATH from TBL_MAS_FILE_UPLOADS where FILE_CONTENT_TYPE LIKE 'image/'+'%' AND FILE_REGNO = @REGNO and FILE_TYPE = 'VehicleImage' 
		ORDER BY 
				FILE_NAME ASC
